...
```{r}
#> Ratings with room type
ggplot(airbnb, aes(rating, fill = room_type)) +
  geom_bar()
```

```{r}
#> See the types of rooms in each district
...
ggplot(airbnb, aes(as.factor(rating), price)) +
  geom_boxplot() +
  facet_wrap(~district)
```
...
```{r}
#> Peer plot
ggplot(airbnb, aes(district, price, color = room_type)) +
  geom_boxplot()
# people like renting homes more than rooms
# people dont like sharing rooms/ there are no shared rooms in central far or southwest
# The most expensive rentals are in the North 
```
...
airbnb_model <- stan_glmer(
  log(price) ~ walk_score + bedrooms + rating + room_type +
    (1 | neighborhood), 
  data = airbnb, family = gaussian, #gaussian = normal distribution (which we figured out above)
