...
```{r}
## Vis
ggplot(data = fake_news,
       aes(x = type, fill = title_has_excl)) +
  geom_bar()
```
...
```{r}
prior_probability_article_excl <- 0.12
```
...
(prior_probability_article_fake *
    likelihood_article_fake_has_excl) / prior_probability_article_excl
```
...
```{r}
likelihood_article_fake_has_excl <- 0.27
```
...
```{r}
# region and what they use (percentages)
pop_vs_soda %>%
  tabyl(word_for_cola, region) %>%
  adorn_percentages("col")

#### For next class, figure out how to search results for the word "pop" ONLY
```
...
```{r}
ggplot(data = pop_vs_soda, aes(x = region, fill = pop)) + geom_bar()
...
### Marginal Prior probability that a person uses the word pop
