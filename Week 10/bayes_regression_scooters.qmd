...
```{r}
library(plotly)
X <- ggplot(bikes_scooters, aes(x = `temp_feel`, y = `scooter_rides`, color = `day_of_week`)) +
  geom_point() +
...
avg_scooter_rides_by_day <- bikes_scooters %>%
  ...
  summarize(mean_rides = mean(scooter_rides, na.rn = TRUE))

ggplot(avg_scooter_rides_by_day, aes(x = day_of_week, y = mean_rides)) +
...
ggplot(bikes_scooters) +
  geom_point(aes(temp_feel, rides, color = "coral")) +
  geom_point(aes(temp_feel, scooter_rides, color = "steelblue"))
```
...
Lydia's example - Old Navy bringing back old trends. They can use old data to make business decisions on what to bring back/ how much to make.
