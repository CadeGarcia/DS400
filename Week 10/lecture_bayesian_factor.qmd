...
```{r}
ggplot(bikes, aes(weekend, rides, fill = weekend)) +
  geom_boxplot() +
...
```{r}
ggplot(bikes, aes(rides, fill = weekend)) + 
  geom_density(alpha = 0.5) # does not need Y axis bc the Y is the density
```
...
df %>%
  group_by(DRD370B) %>%
  summarise(methyl_mean = mean(LBXBGM))
```

-   Show difference with a boxplot

```{r}
ggplot(df, aes(DRD370B, LBXBGM, fill = DRD370B)) +
  geom_boxplot()
```

```{r}
ggplot(df, aes(LBXBGM, fill = DRD370B)) +
  geom_histogram()
```
...
```{r}
ttestBF(formula = LBXBGM ~ DRD370B,  # Numeric variable on left, categorical on right
        data = df)

#> 606.6176 times more likely that there will be a difference between the 2 groups in methyl mercury levels
#> BF10 > 100 is very strong evidence  
```
