...
```{r}
plot <- ggplot(bikes, aes(temp_actual, rides, color = day_of_week)) +
  geom_point() +
  labs(title = "Demand for bikeshares")

ggplotly(plot)
```

```{r}
x <- ggplot(data = bikes, aes(x = temp_actual, y = rides, color = day_of_week)) +
  geom_point() +
  theme("minimal") +
facet_wrap(~season) # makes 4 graphs based on what column you choose!
...
```{r}
ggplot(bikes, aes(day_of_week, rides, fill = as.factor(year))) +
  geom_col() +
  facet_wrap(~season)
```
...
```{r}
ggplot(bikes, aes(temp_feel, rides)) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor() # how are they correlated? *printed at the top left of the screen*
```
...
```{r}
ggplot(bikes, aes(day_of_week, rides, color = windspeed)) +
  geom_point()

#playing around with columns to view whats good and whats bad
```

#### WORST DAY FOR BIKING!
